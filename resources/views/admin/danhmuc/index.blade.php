    ...
    <div class="mb-6 grid grid-cols-2 gap-6">
      <div>
        ...
        <form action="{{ route('danhmuc.store') }}" method="POST" class="flex gap-2">
          @csrf
          <input name="ten_danh_muc" placeholder="Tên danh mục" class="border rounded px-3 py-2 w-1/2" required />
          <input name="ten_the_loai" placeholder="(Tùy chọn) Tên thể loại khởi tạo" class="border rounded px-3 py-2 w-1/3" />
      ...
      <div>
        ...
        <form action="{{ route('danhmuc.theloai.store') }}" method="POST" class="flex gap-2">
          @csrf
          <select name="ma_danh_muc" class="border rounded px-3 py-2 w-1/3" required>
            <option value="">Chọn danh mục</option>
            @foreach($danhmucs as $opt)
            <option value="{{ $opt->ma_danh_muc }}">{{ $opt->ten_danh_muc }}</option>
            @endforeach
          </select>
          <input name="ten_the_loai" placeholder="Tên thể loại" class="border rounded px-3 py-2 w-1/3" required />
          <button class="px-4 py-2 bg-green-600 text-white rounded" style='background-color: #10B981;'>Thêm thể loại</button>
        </form>
        @error('ten_the_loai')
        <div class="text-red-600">{{ $message }}</div>
        @enderror
            ...
            <table class="w-full whitespace-no-wrap mb-4">
              <thead>
                <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase bg-gray-50">
                  <th class="px-4 py-3">Mã thể loại</th>
              ...
              <tbody class="bg-white divide-y">
                ...
                <tr class="text-gray-700">
                  <td class="px-4 py-3">{{ $tl->ma_the_loai }}</td>
                  <td class="px-4 py-3">
                    <a href="{{ route('danhmuc.theloai.sanpham', $tl->ma_the_loai) }}" class="text-blue-600">{{ $tl->ten_the_loai }}</a>
                  </td>
                  <td class="px-4 py-3">{{ optional($tl->danhmuc)->ten_danh_muc }}</td>
                  <td class="px-4 py-3">
                    <a href="{{ route('danhmuc.theloai.edit', $tl->ma_the_loai) }}" class="text-blue-600 mr-2">Sửa</a>
                    <form action="{{ route('danhmuc.theloai.destroy', $tl->ma_the_loai) }}" method="POST" style="display:inline">
                      @csrf
                      @method('DELETE')
                      <button onclick="return confirm('Xóa thể loại?')" class="text-red-600">Xóa</button>
                    </form>
                  </td>
                </tr>
                @endforeach
      ...
      <div class="w-full overflow-x-auto">
        <table class="w-full whitespace-no-wrap">
          ...
          <tbody class="bg-white divide-y">
            ...
            <tr class="text-gray-700">
              <td class="px-4 py-3">{{ $dm->ma_danh_muc }}</td>
              <td class="px-4 py-3">{{ $dm->ten_danh_muc }}</td>
              <td class="px-4 py-3">
                <a href="{{ route('danhmuc.edit', $dm->ma_danh_muc) }}" class="text-blue-600 mr-2">Sửa</a>
                <form action="{{ route('danhmuc.destroy', $dm->ma_danh_muc) }}" method="POST" style="display:inline">
                  @csrf
                  @method('DELETE')
                  <button onclick="return confirm('Xóa danh mục?')" class="text-red-600">Xóa</button>
                </form>
              </td>
            </tr>
            @endforeach
